...
describe 'mongodb_sharding' do
  ...
  context 'configserver role' do
    ...
    it do
      expect(chef_run).to create_mongodb_instance('mongod')
        .with(replicaset: true, replicaset_name: 'configReplSet')
    end
  end
  ...
  context 'mongos role' do
    ...
    it do
      expect(chef_run).to create_mongodb_instance('mongos')
        .with(service_actions: [:enable])
    end
  end
end
